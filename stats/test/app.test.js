...
describe("HTTP request handler", () => {
  ...
  describe("GET /retrieval-success-rate", () => {
    ...
    it("returns today stats for no query string", async () => {
      const day = today();
      await givenRetrievalStats(pgPools.evaluate, {
        day,
        total: 10,
        successful: 1,
        successfulHttp: 0,
        successfulHttpHead: 0,
      });
      const res = await fetch(new URL("/retrieval-success-rate", baseUrl), {
        redirect: "follow",
      });
      await assertResponseStatus(res, 200);
    ...
    it("caches data including today for short time", async () => {
      const res = await fetch(
        new URL(
          `/retrieval-success-rate?from=2024-01-01&to=${today()}`,
          baseUrl,
        ),
        {
          redirect: "manual",
    ...
    it("caches historical including for long time & marks them immutable", async () => {
      const res = await fetch(
        new URL(
          "/retrieval-success-rate?from=2023-01-01&to=2023-12-31",
          baseUrl,
        ),
        {
          redirect: "manual",
      ...
      assert.deepStrictEqual(stats, [
        {
          ...
          successful: "51",
          successful_http: "36",
          success_rate_http: 36 / 110,
          success_rate_http_head: 36 / 110,
        },
        {
          day: "2024-01-11",
          success_rate: 61 / 220,
          ...
          successful: "61",
          successful_http: "50",
          success_rate_http: 50 / 220,
          success_rate_http_head: 50 / 220,
        },
      ]);
    });
      ...
      assert.deepStrictEqual(stats, [
        {
          ...
          successful: "5",
          successful_http: "3",
          success_rate_http: 3 / 10,
          success_rate_http_head: 3 / 10,
        },
        {
          day: "2024-01-20",
          success_rate: 1 / 10,
          ...
          successful: "1",
          successful_http: "1",
          success_rate_http: 1 / 10,
          success_rate_http_head: 1 / 10,
        },
      ]);
    });
    ...
    it("preserves additional query string arguments when redirecting", async () => {
      ...
      const res = await fetch(
        new URL("/retrieval-success-rate?nonZero=true", baseUrl),
        { redirect: "follow" },
      );
      await assertResponseStatus(res, 200);
      const stats = await res.json();
    ...
    it("handles successful_http values 0, null, undefined", async () => {
      ...
      assert.deepStrictEqual(stats, [
        ...
        },
        {
          ...
          successful: "1",
          total: "10",
          successful_http: null,
          success_rate_http: null,
          success_rate_http_head: null,
        },
        {
          day: "2024-01-22",
  ...
  describe("GET /participants/daily", () => {
    it("returns daily active participants for the given date range", async () => {
      await givenDailyParticipants(pgPools.evaluate, "2024-01-10", [
        "0x10",
        "0x20",
      ]);
      await givenDailyParticipants(pgPools.evaluate, "2024-01-11", [
        "0x10",
        "0x20",
        "0x30",
      ]);
      await givenDailyParticipants(pgPools.evaluate, "2024-01-12", [
        "0x10",
        "0x20",
        "0x40",
        "0x50",
      ...
      const res = await fetch(
        new URL("/participants/daily?from=2024-01-11&to=2024-01-12", baseUrl),
        {
          redirect: "manual",
        },
      );
  ...
  describe("GET /participants/monthly", () => {
    it("returns monthly active participants for the given date range ignoring the day number", async () => {
      // before the range
      await givenDailyParticipants(pgPools.evaluate, "2023-12-31", [
        "0x01",
        "0x02",
      ]);
      // in the range
      ...
      await givenDailyParticipants(pgPools.evaluate, "2024-02-13", [
        "0x10",
        "0x60",
      ]);
      // after the range
      ...
      const res = await fetch(
        new URL("/participants/monthly?from=2024-01-12&to=2024-02-12", baseUrl),
        {
          redirect: "manual",
        },
      );
  ...
  describe("GET /participants/change-rates", () => {
    it("returns monthly change rates for the given date range ignoring the day number", async () => {
      ...
      await givenDailyParticipants(pgPools.evaluate, "2024-02-11", [
        "0x10",
        "0x20",
      ]);
      await givenDailyParticipants(pgPools.evaluate, "2024-02-13", [
      ...
      await givenDailyParticipants(pgPools.evaluate, "2024-03-11", [
        "0x10",
        "0x20",
      ]);
      await givenDailyParticipants(pgPools.evaluate, "2024-03-13", [
        "0x10",
        "0x60",
      ]);
      // after the range
  ...
  describe("GET /participant/:address/reward-transfers", () => {
    it("returns daily reward transfers for the given date range", async () => {
      await givenRewardTransfer(pgPools.stats, "2024-01-11", "0x00", "1", 0);
      ...
      await assertResponseStatus(res, 200);
      const stats = await res.json();
      assert.deepStrictEqual(stats, [{ day: "2024-01-11", amount: "1" }]);
    });
  });
    ...
    it("returns a summary of miners RSR for the given date range", async () => {
      ...
      assert.deepStrictEqual(stats, [
        ...
        },
        {
          ...
          successful: "150",
          successful_http: "100",
          success_rate_http: 100 / 200,
          success_rate_http_head: 100 / 200,
        },
      ]);
    });
    it("handles successful_http values 0, null, undefined", async () => {
  ...
  describe("summary of eligible deals", () => {
    describe("GET /miner/{id}/deals/eligible/summary", () => {
      it("redirects to spark-api", async () => {
        const res = await fetch(
          new URL("/miner/f0230/deals/eligible/summary", baseUrl),
          { redirect: "manual" },
        );
        ...
        assert.strictEqual(
          res.headers.get("location"),
          "https://api.filspark.com/miner/f0230/deals/eligible/summary",
        );
      });
    });
    ...
    describe("GET /client/{id}/deals/eligible/summary", () => {
      it("redirects to spark-api", async () => {
        const res = await fetch(
          new URL("/client/f0800/deals/eligible/summary", baseUrl),
          { redirect: "manual" },
        );
        await assertResponseStatus(res, 302);
        assert.strictEqual(res.headers.get("cache-control"), "max-age=21600");
        assert.strictEqual(
          res.headers.get("location"),
          "https://api.filspark.com/client/f0800/deals/eligible/summary",
        );
      });
    });
    ...
    describe("GET /allocator/{id}/deals/eligible/summary", () => {
      it("redirects to spark-api", async () => {
        const res = await fetch(
          new URL("/allocator/f0500/deals/eligible/summary", baseUrl),
          { redirect: "manual" },
        );
        await assertResponseStatus(res, 302);
        assert.strictEqual(res.headers.get("cache-control"), "max-age=21600");
        assert.strictEqual(
          res.headers.get("location"),
          "https://api.filspark.com/allocator/f0500/deals/eligible/summary",
        );
      });
    });
  });
  ...
  describe("GET /deals/daily", () => {
    it("returns daily deal stats for the given date range", async () => {
      ...
      const res = await fetch(
        new URL("/deals/daily?from=2024-01-11&to=2024-01-12", baseUrl),
        {
          redirect: "manual",
        },
      );
    ...
    it("aggregates stats over miners", async () => {
      ...
      assert.deepStrictEqual(
        stats.map(({ day, tested }) => ({ day, tested })),
        [
          {
            day: "2024-01-11",
            tested: String(10 + 20),
  ...
  describe("GET /deals/summary", () => {
    it("returns deal summary for the given date range (including the end day)", async () => {
      ...
      const res = await fetch(
        new URL("/deals/summary?from=2024-03-24&to=2024-03-30", baseUrl),
        {
          redirect: "manual",
        },
      );
    ...
    it("handles query for future date with no recorded stats", async () => {
      const res = await fetch(
        new URL("/deals/summary?from=3024-04-24&to=3024-03-30", baseUrl),
        {
          redirect: "manual",
        },
      );
  ...
  describe("miner retrieval timing stats", () => {
    beforeEach(async () => {
      // before the range
      await givenRetrievalTimings(pgPools.evaluate, {
        day: "2024-01-09",
        minerId: "f1one",
        timeToFirstByteP50: [1000],
      });
      await givenRetrievalTimings(pgPools.evaluate, {
        day: "2024-01-09",
        minerId: "f1two",
      ...
      await givenRetrievalTimings(pgPools.evaluate, {
        day: "2024-01-10",
        minerId: "f1one",
        timeToFirstByteP50: [123, 345],
      });
      await givenRetrievalTimings(pgPools.evaluate, {
        day: "2024-01-10",
        minerId: "f1two",
        timeToFirstByteP50: [654, 789],
      });
      // after the range
      await givenRetrievalTimings(pgPools.evaluate, {
        day: "2024-01-21",
    ...
    describe("GET /clients/retrieval-success-rate/summary", () => {
      beforeEach(async () => {
        await pgPools.evaluate.query(
          "DELETE FROM daily_client_retrieval_stats",
        );
      });
      it("returns a summary of clients RSR for the given date range", async () => {
        // before the range
        await givenClientRetrievalStats(pgPools.evaluate, {
          day: "2024-01-10",
          clientId: "f1oneClient",
          total: 10,
          successful: 1,
          successfulHttp: 1,
        });
    ...
    describe("GET /allocators/retrieval-success-rate/summary", () => {
      beforeEach(async () => {
        await pgPools.evaluate.query(
          "DELETE FROM daily_allocator_retrieval_stats",
        );
      });
      it("returns a summary of allocators RSR for the given date range", async () => {
        // before the range
        await givenAllocatorRetrievalStats(pgPools.evaluate, {
          day: "2024-01-10",
          allocatorId: "f1oneAllocator",
          total: 10,
          successful: 1,
          successfulHttp: 1,
        });
 ...
 */
const givenRetrievalStats = async (
  pgPool,
  { day, minerId, total, successful, successfulHttp, successfulHttpHead },
) => {
  await pgPool.query(
 ...
 * @param {number | bigint} [data.successfulHttp]
 */
const givenClientRetrievalStats = async (
  pgPool,
  { day, clientId, total, successful, successfulHttp },
) => {
  await pgPool.query(
 ...
 * @param {number | bigint} [data.successfulHttp]
 */
const givenAllocatorRetrievalStats = async (
  pgPool,
  { day, allocatorId, total, successful, successfulHttp },
) => {
  await pgPool.query(
 ...
 * }} stats
 */
const givenDailyDealStats = async (
  pgPool,
  {
    day,
    minerId,
 ...
 */
const givenRetrievalTimings = async (
  pgPool,
  { day, minerId, timeToFirstByteP50 },
) => {
  await pgPool.query(
